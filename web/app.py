...
import os
import psycopg2
from config import DEV_DB, pg_user, pg_pass, pg_db, pg_host, PROD_DB

# docker.exe run -p 5000:5000 -e DEBUG=1 flask-site
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///enquete.db'
...
#conn = psycopg2.connect("dbname=enquete user=postgres password=postgres")
...
def answer_process():
    ...
    if not request.args:
        return redirect(url_for('question1_page'))
